			...
			for (int i = 0; i <= fdmax; i++) {
				if (FD_ISSET(i, &read_fds)) {
					if (i == listenfd) {
						...
						if (newfd2 != -1) {
							/* add new client */
							wsAddClient(newfd2, cli_addr.sin_addr);
							//Deprecated ntoa API
							printf("New connection from %s on socket %d\n", inet_ntoa(cli_addr.sin_addr), newfd2);
						}
					}
					else if (i == listenfd3) {
						socklen_t addrlen = sizeof(cli_addr);
						...
						if (newfd3 != -1) {
							/* add new client */
							wsAddClient(newfd3, cli_addr.sin_addr);
							//Deprecated ntoa API
							printf("New connection from %s on socket %d\n", inet_ntoa(cli_addr.sin_addr), newfd3);
						}
					}
					else if (i == listenfd4) {
						socklen_t addrlen = sizeof(cli_addr);
						...
						if (newfd4 != -1) {
							/* add new client */
							wsAddClient(newfd4, cli_addr.sin_addr);
							//Deprecated ntoa API
							printf("New connection from %s on socket %d\n", inet_ntoa(cli_addr.sin_addr), newfd4);
